...
class SpaceInvaders:
    ...
    def playerUpdate(self, *args):
        X = []
        X.append(self.playerX)
        activation = self.individual.predict(X)
        if activation[0] == True and self.playerX < 800 - self.player.get_width():
            self.playerX +=5
        if activation[1] == True and self.playerX > 0:
            self.playerX -=5
        if activation[2] == True and not self.bullet:
